...
export default function ClientWelcome() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // compute medium as 100 - easy - hard (parse numbers, allow inputs like '20' or '20%')
    const parsePercent = (s: string) => {
      const n = Number(String(s || '').replace(/[^0-9.-]/g, ''));
      return Number.isFinite(n) ? n : 0;
    };
    const easyNum = parsePercent(difficulty.easy);
    const hardNum = parsePercent(difficulty.hard);
    let mid = 100 - easyNum - hardNum;
    mid = Math.max(0, Math.min(100, Math.round(mid)));
    const computedMedium = `${mid}%`;

    const payload = {
      testName,
      difficulty: {
        // send numeric values without any '%' symbol
        easy: String(easyNum),
        medium: String(mid),
        hard: String(hardNum),
      },
      ...
      <Card className="w-full max-w-4xl animate-fade-in-up">
        ...
        <form onSubmit={handleSubmit}>
          <CardContent className="space-y-6">
            ...
            <div className="flex items-center space-x-4">
                <Label className="whitespace-nowrap">Difficulty Level</Label>
                <div className="flex items-center space-x-2">
                  <Label className="whitespace-nowrap">Easy</Label>
                  <div className="flex items-center">
                    <Input id="difficulty-easy" placeholder="20" value={difficulty.easy} onChange={(e) => setDifficulty({ ...difficulty, easy: e.target.value })} />
                    <span className="ml-2">%</span>
                  </div>
                </div>
                <div className="flex items-center space-x-2">
                  <Label className="whitespace-nowrap">Hard</Label>
                  <div className="flex items-center">
                    <Input id="difficulty-hard" placeholder="30" value={difficulty.hard} onChange={(e) => setDifficulty({ ...difficulty, hard: e.target.value })} />
                    <span className="ml-2">%</span>
                  </div>
                </div>
                <div className="flex items-center space-x-2">
                  <Label className="whitespace-nowrap">Medium</Label>
                  {/* Medium is computed as 100 - easy - hard and is read-only */}
                  <div className="flex items-center">
                    <Input id="difficulty-medium" placeholder="50" value={`${Math.max(0, Math.min(100, 100 - Number(String(difficulty.easy).replace(/[^0-9.-]/g, '') || 0) - Number(String(difficulty.hard).replace(/[^0-9.-]/g, '') || 0)))}`} disabled />
                    <span className="ml-2">%</span>
                  </div>
                </div>
            </div>
